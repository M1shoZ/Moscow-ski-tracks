...
// Обработка формы с фильтрами и поиском
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $filter_sql = "SELECT * FROM `tracks` WHERE 1";

    if ($equipment !== 'Нет') {
        $filter_sql .= " AND HasEquipmentRental = 'Да'";
    }

    if ($maintenanceService !== 'Нет') {
        $filter_sql .= " AND HasTechService = 'Да'";
    }

    if ($dressingRoom !== 'Нет') {
        $filter_sql .= " AND HasDressingRoom = 'Да'";
    }
    if ($HasEatery !== 'Нет') {
        $filter_sql .= " AND HasEatery = 'Да'";
    }
    if ($HasWifi !== 'Нет') {
        $filter_sql .= " AND HasWifi = 'Да'";
    }
    if ($HasFirstAidPost !== 'Нет') {
        $filter_sql .= " AND HasFirstAidPost = 'Да'";
    }
    if ($surfaceType !== '') {
        $filter_sql .= " AND SurfaceTypeWinter = '$surfaceType'";
    }
    if ($paid !== '') {
        $filter_sql .= " AND paid = '$paid'";
    }

    // Добавляем условие поиска, если есть текст поиска
    if (!empty($searchTerm)) {
        $filter_sql .= " AND (ObjectName LIKE '%$searchTerm%' OR Address LIKE '%$searchTerm%')";
    }

    // Добавляем LIMIT и OFFSET
    $offset = ($currentPage - 1) * $perPage;
    $filter_sql .= " LIMIT $offset, $perPage";

    // Выполняем запрос
    $result = mysqli_query($link, $filter_sql);

    // Вывод результатов с использованием функции displayTracks
...
<body>
    ...
    <div class="container mt-5">
        <!-- Форма с фильтрами -->
        <form id="filterForm">
            <div class="row mb-3"ы>
                <h1>Поиск трасс</h1>
                <!-- Введите поле для поиска -->
                <div class="col-md-9 mt-2 mb-3">
                    <input type="text" class="form-control" name="searchTerm"
                        placeholder="Введите название или адрес трассы">
                </div>
                <div class="col-md-3">
    ...
    <div class="container table-responsive">
        <table class="table mx-auto" id="recordsTable">
            <!-- Заголовки таблицы -->
            <thead>
                <tr>
